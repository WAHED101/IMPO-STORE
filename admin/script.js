...
function loadRecentActivity(limit = 10) {
  ...
  const deletionPromise = new Promise(resolve => {
    ...
    try {
      ...
      // Use sessionStorage as a more reliable alternative on GitHub Pages
      let deletionHistory = [];
      try {
        deletionHistory = JSON.parse(localStorage.getItem('deletionHistory') || '[]');
      } catch (e) {
      ...
      // If we have a URL parameter 'showDeletedDemo=true', add a demo deleted item
      // This helps with GitHub Pages debugging
      const urlParams = new URLSearchParams(window.location.search);
      if (urlParams.get('showDeletedDemo') === 'true') {
        deletionHistory.push({
          id: 'demo-deletion-' + Date.now(),
          category: 'request',
          name: "Demo user's request for Demo Item (Demo Price)",
          timestamp: Date.now() - 60000 // 1 minute ago
        });
        console.log('Added demo deletion item');
      }
      
      deletionHistory.forEach(deletion => {
        // Create a more specific title based on category
        let title = `Deleted ${deletion.category || 'item'}: ${deletion.name || 'Unknown item'}`;
        let icon = 'fa-trash-alt';
        let iconClass = 'activity-error';
        
        // Special case for requests
        if (deletion.category === 'request') {
          title = `Deleted Request: ${deletion.name}`;
          icon = 'fa-trash-alt';
        ...
        deletions.push({
          ...
          type: 'deletion',
          title: title,
          description: `${deletion.category === 'request' ? 'Purchase request' : deletion.category} was deleted from the system`,
